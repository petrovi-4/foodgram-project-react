...
class UserViewSet(ListCreateRetrieveModelMixin):
    """Вывод списка и создание экземпляра пользователя."""
    ...
    def subscribe(self, request, pk):
        ...
        if request.user == author:
            raise SubscribeException('Подписаться на самого себя нельзя!')
        if is_subscribed(request.user, author.id):
            raise SubscribeException('Подписка уже оформлена.')
        follow = Follow.objects.create(user=request.user, author=author)
        follow.save()
        serializer = serializers.MySubscriptionSerializer(
    ...
    def delete_subscribe(self, request, pk):
        ...
        if not is_subscribed(request.user, pk):
            raise SubscribeException(f'Вы не подписаны на {author}')
        follow.delete()
...
class GetTokenView(APIView):
    ...
    permission_classes = (AllowAny,)
    serializer_class = serializers.GetToketSerializer
